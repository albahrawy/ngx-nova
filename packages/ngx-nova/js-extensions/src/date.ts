...
export namespace dates {
    ...
    function _parseTimePart(value?: string, max = 59): number | undefined {
        ...
        if (!isNaN(t) && numbers.between(t, 0, max))
            return t;
        return 0;
    }
    ...
    function _parseMonth(month?: string): number | undefined {
        ...
        if (!isNaN(m) && numbers.between(m, 0, 11))
            return m;
        return undefined;
    }
    ...
    function _parseHour(hour?: string, hour24?: string, ampm?: string): number | undefined {
        if (hour && !hour24) {
            ...
            if (!isNaN(h) && numbers.between(h, 0, 23))
                return h;
        } else if (hour24) {
            const h24 = Number(hour24);
            if (!isNaN(h24) && numbers.between(h24, 0, 23))
                return h24;
        }
        return 0;
    }
    ...
    function _parseFromJson(value: string) {
        ...
        if (tokens) {
            if (!numbers.between(+tokens[2] - 1, 1, 11))
                return null;
            if (!numbers.between(+tokens[3], 1, daysInMonth(+tokens[2], +tokens[1])))
                return null;
            if (!numbers.between(+tokens[4], 0, 59) || !numbers.between(+tokens[5], 0, 59) || !numbers.between(+tokens[6], 0, 59))
                return null;
    ...
    function _parseExactCore(stringValue: string, formatString: string, preserveTime?: boolean): Date | null {
    ...
    function _getLocalInfo(key: 'dayNames' | 'monthNames', style: 'long' | 'short' | 'narrow', locale?: string): string[] | undefined {
        return _getLocalInfoCore(key)?.[style];
    }
     ...
     */
    export function registerLocale(code: string, info: IDateLocale): void {
        if (!code || !info)
     ...
     */
    export function parseExact(value: string, parseFormat: string, preserveTime?: boolean): Date | null {
     ...
     * @returns {Date | null} - The parsed date, or null if parsing fails.
     */
    export function parse(value: any, parseFormat?: string | string[], preserveTime?: boolean): Date | null {
        if (!value)
            return null;
        ...
        if (is.string(value)) {
            ...
            if (!dateValue || !isValidDate(dateValue)) {
                ...
                for (const currentFormat of parseFormats) {
                    dateValue = parseExact(value, currentFormat);;
                    if (!!dateValue && isValidDate(dateValue))
                        break;
                }
            }
     ...
     */
    export function format(dateValue: Date, formatString?: string, locale?: string): string | null {
        if (!dateValue || !isValidDate(dateValue))
            return null;
    ...
    export function getMonthNames(style: 'long' | 'short' | 'narrow' = 'long', locale?: string): string[] {
        ...
        return arrays.enumerableRange(0, 11).map(m => formatter.format(new Date(2017, m, 1)));
    }
    ...
    export function getDateNames(locale?: string): string[] {
        ...
        return arrays.enumerableRange(1, 31).map(d => formatter.format(new Date(2017, 0, d)));
    }
    ...
    export function getDayOfWeekNames(style: 'long' | 'short' | 'narrow' = 'long', locale?: string): string[] {
        ...
        return arrays.enumerableRange(0, 7).map(d => formatter.format(new Date(2017, 0, d)));
    }
    ...
    export function daysInMonth(month: number, year?: number): number {
        return numbers.between(month, 1, 12) ? (daysInMonths[month - 1] ?? (isLeapYear(year) ? 29 : 28)) : NaN;
    }
     ...
     */
    export function closesDate(date: Date, datesArray: Array<Date>): Date | null {
        if (!isValidDate(date))
            return null;
     ...
     */
    export function compare(first: Date, second: Date): 1 | -1 | 0 {
     ...
     */
    export function different(first: Date, second: Date, interval: 'd' | 'M' | 'y' | 'h' | 'm' | 's' | 'ms' = 'd'): number {
        if (!isValidDate(first) || !isValidDate(second))
            throw new Error("Invalid dates");
